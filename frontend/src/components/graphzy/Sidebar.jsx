...
import { Layers, Clock, LayoutGrid, User, BookOpen, ArrowLeft } from 'lucide-react';
...
export default function Sidebar({ activeView, onViewChange }) {
  ...
  return (
    <aside className="w-full md:w-[68px] bg-[#FAFAF8]/92 backdrop-blur-md border-b md:border-b-0 md:border-r border-black/[0.06] flex md:flex-col items-center justify-around md:justify-start py-3 md:py-6 gap-2 md:gap-6 sticky top-[54px] h-auto md:h-[calc(100vh-54px)] z-10">
      ...
      <Link
        to="/graphzy"
        className="flex flex-col items-center gap-1 cursor-pointer bg-transparent border-none w-12 py-2 rounded-lg text-[#A3A3A3] hover:text-[#525252] hover:bg-black/[0.02] duration-150 md:mt-auto no-underline"
      >
        <ArrowLeft size={20} className="stroke-[#A3A3A3]" />
        <span className="text-[8px] font-mono font-semibold uppercase tracking-wider text-[#A3A3A3]">
          Exit
        </span>
      </Link>
    </aside>
